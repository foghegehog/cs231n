...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        affine, affine_cache = temporal_affine_forward(h, W_vocab, b_vocab)
        ...
        grads['W_embed'] = word_embedding_backward(dword_vec, embed_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        words_batch = self._start * np.ones((N, )).astype(np.int)
        h = features @ W_proj + b_proj
        c = np.zeros_like(h)
        captions_index = 0
        for t in range(max_length):
            words_vec = W_embed[words_batch, :]
            if self.cell_type == 'rnn':
            ...
            words_batch = np.argmax(scores, axis=2).reshape((N,))
            captions[:, captions_index] = words_batch
            captions_index +=1
